...
- Better Auth manages the full authentication flow under `/api/auth/*`:
  - Registration, login, logout, and any other auth-related actions are provided by Better Auth’s own handlers.
  - It stores users and sessions in the `user`, `session`, `account`, and `verification` tables defined in the Drizzle schema.
- The frontend should:
  - Use Better Auth’s client flows (or direct HTTP calls to `/api/auth/*`) for signup, login, and logout.
  - Ensure that auth cookies set by Better Auth are sent on subsequent API requests (e.g. `fetch(url, { credentials: "include" })`).
- On each request to `/api/meal-plans/*`:
  - `requireAuth` calls `auth.api.getSession(...)` with the incoming headers.
  - If no valid session is found: returns `401 { "error": "Unauthorized" }`.
  - If valid: attaches `user` and `session` to the Hono context.

For the frontend, the key point: **Treat all `/api/meal-plans/*` routes as “logged-in only”, and make sure Better Auth’s cookies are sent on each request.**
...
All endpoints below require a valid Better Auth session (auth cookies).
...
### 5.5 Replace a single meal entry (AI-generated alternative)
...
- Use Better Auth for signup/login/logout under `/api/auth/*` (via its SDK or direct HTTP).
- Always send credentials before calling `/api/meal-plans`:
  - `fetch(url, { credentials: "include" })` so Better Auth cookies are included.
- For **creating** a plan:
